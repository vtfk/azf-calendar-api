...
module.exports = async (context, req) => {
  ...
  let userPrincipalName, graphUser, graphOrg
  ...
  try {
    ...
    context.log.error(['events', 'graph-user', 'err', err])
  ...
  try {
    ...
    if (userTenantId !== tenantId) {
      context.log.error(['events', userPrincipalName, `Tenant ID not matching ${tenantId}`, userTenantId])
      throw new HTTPError(401, 'Invalid tenant ID')
    }
  } catch (err) {
    context.log.error(['events', userPrincipalName, 'graph-org', 'err', err])
